...
- **Service-account PersonId Resolution (Path F + G + D)**: Three resolution paths are available for the TxSubscriber echo-filter identity. **Path F** (operator-provided, `SERVICE_ACCOUNT_PERSON_ID` env var): UUID used directly, `SERVICE_ACCOUNT_RESOLVED=1`. **Path G** (boot-time probe, automatic): pod writes a sentinel doc and reads back `tx.modifiedBy`; success sets `SERVICE_ACCOUNT_RESOLVED=1`; bounded to 10 s with clean fallback. **Path D** (sentinel fallback): if both F and G fail, falls back to `systemAccountUuid` cast with `SERVICE_ACCOUNT_RESOLVED=0` gauge for operator alerting. Echo defense uses dual-layer matching: (layer 1) `tx.modifiedBy === serviceAccountPersonId` filter, and (layer 2) `_originated: 'gitlab'` marker stamped by 7 sync managers across `attributes`, `operations`, and `operations.$set`. `TxRemoveDoc` has no payload and relies on layer-1 alone; operators MUST alert on `tx.subscription.echo.dropped` rate. See [Phase 5 Runbook — Service-Account PersonId Resolution](docs/phase5-runbook.md#service-account-personid-resolution) for operator procedures.
